...
class sms {
    ...
    * @var integer
    */
    var $api_id;// = $smsVars["smsAPI"];

    /**
    * Clickatell username
    * @var mixed
    */
    var $user;// = $smsVars["smsAccount"];

    /**
    * Clickatell password
    * @var mixed
    */
    var $password ;//= $smsVars["smsPassword"];
    ...
    * @access public
    */
    function sms ($smsVars) {
     $this->password = $smsVars["smsPassword"];
     $this->user = $smsVars["smsAccount"];
     $this->api_id = $smsVars["smsAPI"];
        if ($this->use_ssl) {
    ...
    function send($to=null, $from=null, $text=null) {
        ...
        if ($this->unicode == true) {
            ...
            if (mb_strlen ($text) > 210) {
        	    echo ("Your unicode message is too long! (Current lenght=".mb_strlen ($text).")");
        	}
            ...
            if (strlen ($text) > 459) {
    	        echo ("Your message is too long! (Current lenght=".strlen ($text).")");
    	    }
        	/* Does message need to be concatenate */
            if (strlen ($text) > 160) {
                $concat = "&concat=3";
    ...
    */
    function _parse_send ($result) {
    	$code = substr($result, 0, 2);
    	if ($code!="ID") {
    	    echo ("<strong>Error while sending sending the SMS! </br>Please alert the site admin</strong> ");
    	} else {
    	    $code = "OK";
    	}
        return $code;
    ...
    */
    function _chk_mbstring() {
        if (!extension_loaded('mbstring')) {
            echo ("Error. This SMS API class is setup to use Multibyte String Functions module - mbstring, but module not found. Please try to set unicode=false in class or install mbstring module into PHP.");
        }
    }
